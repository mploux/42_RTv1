...
/*   Updated: 2017/04/11 20:27:09 by mploux           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		calc_shadow(t_data *data, t_light light, t_hit *hit)
{
	...
	shadow_hit = throw_ray(data, nray(hit->pos, light_dir));
	if (!shadow_hit)
		return (1);
	shadow_dist = shadow_hit->dist;
	hit_free(&shadow_hit);
...
t_vec3	calc_light(t_data *data, t_light light, t_hit *hit)
{
	...
	light_intensity += calc_specular(data, light_dir, hit) * light_intensity;
	light_color = vec3_mul_d(light.color, light_intensity);
	// t_vec3 normal_color = vec3_add(vec3_mul_d(hit->normal, 0.5), vec3(0.5, 0.5, 0.5));
	// printf("AFTER:%f\n\n", light_intensity);
	return (light_color);
}
